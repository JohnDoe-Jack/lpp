...
static int pVar(bool doAddressLoad)
{
  Symbol symbol;
  if (procname != NULL) {
    char key[256];
...
{
  isAddress = true;
  if (pVar(false) == ERROR) return ERROR;
  isAddress = false;
  genCode("PUSH", "0,GR1");
...
static Obj pFactor(bool doAddressLoad)
{
  ...
  switch (cur->id) {
    ...
    case TNAME:
      factor->isLVal = true;
      if ((factor->type = pVar(doAddressLoad)) == ERROR) return NULL;
      break;
    // 定数
    case TNUMBER:
      factor->type = TPINT;
    ...
    case TLPAREN:
      consumeToken();
      if ((factor = pExpression(doAddressLoad)) == NULL) return NULL;
      if (cur->id != TRPAREN) {
    ...
    case TINTEGER:
      ...
      consumeToken();
      if ((expression = pExpression(doAddressLoad)) == NULL) return NULL;
...
static Obj pTerm(bool doAddressLoad)
{
  int opr;
  Obj factor;
  // 式の結果はGR1に格納されている
  ...
  while (isMulOp(cur->id)) {
    ...
    opr = cur->id;
    consumeToken();
    if (pFactor(doAddressLoad) == NULL) return NULL;
    genCode("POP", "GR2");
    if (opr == TSTAR) {
...
static Obj pSimpleExpression(bool doAddressLoad)
{
  Obj term;
  if (cur->id == TMINUS) {
    consumeToken();
    // 次の項の値に-1を乗じる
    // 式の結果はスタックに積まれている
    if ((term = pTerm(doAddressLoad)) == NULL) return NULL;
    genCode("LAD", "GR2,1");
    genCode("XOR", "GR1,GR1");
...
static Obj pExpression(bool doAddressLoad)
{
  Obj expression;
  int label1, label2;
  // 計算結果はGR1に格納されている
  ...
  while (isRelOp(cur->id)) {
    ...
    consumeToken();
    // 計算結果はGR1に格納されている
    pSimpleExpression(doAddressLoad);
    genCode("POP", "GR2");
    genCode("CPA", "GR2,GR1");
...
static int pInput()
{
  ...
  consumeToken();
  isAddress = true;
  if ((var_type = pVar(false)) == TPRERROR) return ERROR;
  isAddress = false;
...
static int pOutputFormat()
{
  ...
  Obj expression = pExpression(false);
  int output_num = 0;
  if (cur->id != TCOLON) {
